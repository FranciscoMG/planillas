...
$jornada_docenteAdministrativo= isset($_POST['cboJornadaDocenteConPermiso2'])?$_POST['cboJornadaDocenteConPermiso2']:"";
$jornada_docenteAdministrativo = convertirFraccionesDoble($jornada_docenteAdministrativo);
...
if (isset($_POST['btnModificar2'])) {
  ...
  if ($cedula != "") {
    ...
    while ($fila = mysqli_fetch_assoc($resultado)) {
      if ($fila['id_presupuesto'] == $fk_presupuesto) {
        if ($fila['tiempo_sobrante'] < $jornada_docenteAdministrativo) {
          $_SESSION['alerta-contenido'] = "Este presupuesto solo tiene ".$fila['tiempo_sobrante']." tiempos, no puede agregar los ".$jornada_docenteAdministrativo." tiempos del docente.";
          exit();
        }
      }
    }
    ...
    while ($fila4 = mysqli_fetch_assoc($resultado4)) {
      if ($fila4['cedula'] == $cedula) {
          $jornada_docenteAdministrativo2 = $fila4['jornada_docenteAdministrativo'];
          $fk_presupuesto2 = $fila4['fk_presupuesto'];
      }
    }
    ////////////////////

    $seRealizo= $db->modificarDocenteAdministrativo($cedula, $nombre, $apellidos, $grado_academico, $tipo_contrato , $fk_presupuesto , $jornada_docenteAdministrativo);
  ...
  if ($seRealizo) {
    ...
    $tiempo_sobrante2 = ($tiempo_sobrante2 + $jornada_docenteAdministrativo2);
    ...
    //////// Restar al presupuesto //------------
      ...
      $tiempo_sobrante = ($tiempo_sobrante - $jornada_docenteAdministrativo);
      $resultado3 = $dbPresupuesto->restarPresupuesto($fk_presupuesto , $tiempo_sobrante);
      /////////////////////////////////-----------------

      $_SESSION['alerta'] = 2;
      $_SESSION['alerta-contenido'] = "Docente modificado con éxito. <br/> Se agregó ".$jornada_docenteAdministrativo2." tiempos al presupuesto ".$nombre_presupuesto1.".<br/>Se restó ".$jornada_docenteAdministrativo." tiempos del presupuesto ".$nombre_presupuesto2.".";

    ////////////////// Registro de actividad /////////////
    $descripcionRegistroActividad="Se modificó el docente cédula ".$cedula.", ".$nombre." ".$apellidos.". <br/>Se agregó ".$jornada_docenteAdministrativo2." tiempos al presupuesto ".$nombre_presupuesto1.".<br/>Se restó ".$jornada_docenteAdministrativo." tiempos del presupuesto ".$nombre_presupuesto2.".";
...
if (isset($_POST['btnEliminar2'])) {
  ...
  if ($cedula != "") {
    ...
    while ($fila4 = mysqli_fetch_assoc($resultado4)) {
      if ($fila4['cedula'] == $cedula) {
          $jornada_docenteAdministrativo = $fila4['jornada_docenteAdministrativo'];
  ...
  if ($seRealizo) {
    ...
    $tiempo_sobrante2 = ($tiempo_sobrante2 + $jornada_docenteAdministrativo);
...
if (isset($_POST['btnRegistrar2'])) {
  ...
  if ($cedula == "") {
    ...
    $seRealizo = $db->agregarDocenteAdministrativo($cedula, $nombre, $apellidos, $grado_academico, $tipo_contrato , $fk_presupuesto , $jornada_docenteAdministrativo);
    ...
    if (!$seRealizo) {
      ...
      $_SESSION['alerta'] = 2;
			$_SESSION['alerta-contenido'] = "Docente agregrado con éxito <br/>y asignado al presupuesto ".$nombre_presupuesto." con ".$jornada_docenteAdministrativo." tiempos.";

      ////////////////// Registro de actividad /////////////
      $descripcionRegistroActividad="Se agregó el docente cédula ".$cedula.", nombre ".$nombre." ".$apellidos.", asignado al presupuesto ".$nombre_presupuesto." con ".$jornada_docenteAdministrativo." tiempos.";
      $dbRegistroActividad->agregarRegistroActividad($utc, $fecha , $usuario , $descripcionRegistroActividad);
      /////////////////////////////////////////////
...
if (isset($_GET['id'])) {
  ...
  while ($fila = mysqli_fetch_assoc($resultado)) {
      ...
      header("Location: ../masterPage.php?modalDocenteAdministrativo=1&cedula=".$fila['cedula']."&nombre=".$fila['nombre']."&apellidos=".$fila['apellidos']."&grado=".$fila['grado_academico']."&contrato=".$fila['tipo_contrato']."&jornada=".$fila['jornada_docenteAdministrativo']."&jornada_fraccion=".convertirDobleFraciones($fila['jornada_docenteAdministrativo'])."&fk_presupuesto=".$fila['fk_presupuesto']);
  		exit();
  	}
  }
}
