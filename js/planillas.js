...
function FraccionToDoble(fraccion){
	...
	function fraccionADecimalPresupuesto(fraccion) {
		var tamano = fraccion.length;


		if(tamano == 1 || tamano == 2) {
			var final = Number(fraccion);
		}
		if(tamano == 3) {
			switch (fraccion) {
			case '1':
		...
		if(tamano == 5) {
			var double = Number(fraccion.substring(0, 1));
			fraccion = fraccion.substring(2, 3);
		...
		if(tamano == 6) {
			double = Number(fraccion.substring(0, 2));
			fraccion = Number(fraccion.substring(3, 3));
